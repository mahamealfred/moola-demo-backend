...
export const ecoCashDepositService = async (req, res,responseCyclos,description,agent_id) => {
  const {
    sendername,
    senderphone,
    senderaccount,
  ...
  try {
    ...
    logger.info("EcoCash deposit request payload", { requestId, payload });
    ...
    logger.info("EcoCash deposit response received", { requestId, responseData });
    ...
    logger.warn("Cash deposit failed", { 
      requestId, 
    ...
    return res.status(400).json({
      success: false,
      message: responseData?.header?.responsemessage || "Cash deposit failed",
      code: responseData?.header?.responsecode
    });

  } catch (error) {
    logger.error("EcoCash deposit error occurred", {
